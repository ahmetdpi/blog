        ...
        <form action="{{ route('admin.posts.update', $post->id) }}" method="POST">
            ...
            <div style="background:#151525; border:1px solid rgba(99,102,241,0.18); border-radius:14px; overflow:hidden; margin-bottom:20px;">
                ...
                <div style="padding:16px 22px; border-bottom:1px solid rgba(99,102,241,0.18); display:flex; align-items:center; gap:10px;">
                    ...
                    <div>
                        <div style="font-size:13px; font-weight:500; color:#c7d2fe;">Yazıyı Düzenle</div>
                        <div style="font-size:11px; color:#64748b;">{{ $post->title }}</div>
                    </div>
                </div>
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr;">
                    ...
                    <div style="padding:14px 22px; border-bottom:1px solid rgba(99,102,241,0.18); border-right:1px solid rgba(99,102,241,0.18); display:flex; flex-direction:column; gap:6px;">
                        ...
                        <input type="text" name="title" value="{{ old('title', $post->title) }}" style="background:#0f0f1a; border:1px solid rgba(99,102,241,0.18); border-radius:7px; padding:8px 12px; font-size:13px; color:#f1f0ff; outline:none; width:100%; box-sizing:border-box;">
                    </div>
                    ...
                    <div style="padding:14px 22px; border-bottom:1px solid rgba(99,102,241,0.18); display:flex; flex-direction:column; gap:6px;">
                        ...
                        <select name="country" style="background:#0f0f1a; border:1px solid rgba(99,102,241,0.18); border-radius:7px; padding:8px 12px; font-size:13px; color:#f1f0ff; outline:none; width:100%; box-sizing:border-box;">
                            ...
                            <option value="vietnam" {{ old('country', $post->country) == 'vietnam' ? 'selected' : '' }}>VİETNAM</option>
                            <option value="thailand" {{ old('country', $post->country) == 'thailand' ? 'selected' : '' }}>THAİLAND</option>
                            <option value="cambodia" {{ old('country', $post->country) == 'cambodia' ? 'selected' : '' }}>CAMBODİA</option>
                            <option value="maleysia" {{ old('country', $post->country) == 'maleysia' ? 'selected' : '' }}>MALEYSİA</option>
                        </select>
                    </div>
                    ...
                    <div style="padding:14px 22px; grid-column:1/-1; display:flex; flex-direction:column; gap:6px;">
                        ...
                        <textarea name="content" rows="10" style="background:#0f0f1a; border:1px solid rgba(99,102,241,0.18); border-radius:7px; padding:8px 12px; font-size:13px; color:#f1f0ff; outline:none; width:100%; box-sizing:border-box; resize:vertical; font-family:inherit; line-height:1.6;">{{ old('content', $post->content) }}</textarea>
                    </div>
